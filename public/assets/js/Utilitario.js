...
function Eventos() {
    ...
    $('.btnRecetaEme').on('click', async function () {
        ...
        let atencion  = await OrdenMedica.SeleccionarAtencion(Variables.IdCuentaAtencion);
        console.log('atencion',atencion)
        if (idBtnReceta.trim() === '#farmaciaReceA4'){
            
            if (servicios.includes(String(atencion.datos.IdServicio))) {
                $.ajax({
                    url: '/Recetas/informes/generarPDFA4_Observa/' + idReceta,
                    method: 'GET',
                    success: function (response) {
                        if (response.pdf_url) {
