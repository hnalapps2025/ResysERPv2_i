...
var OrdenMedica = {
    ...
    IniciarScript() {
        ...
        OrdenMedica.Eventos(); 
        OrdenMedica.cargarMedicamentos(59);  //Hospi 
        $("#cboFarmacia").val(59).trigger("chosen:updated");
        OrdenMedica.validarEsConsultar();
    ...
    async ObtenerLaboraEmpleado() {
     ...
     validarEsConsultar(){
        const modo = OrdenMedica.getQueryParam("modo");
        ...
        if (modo === "consultar") {
            ...
            document.querySelectorAll('.btnEliminarDiagnostico').forEach(el => {
    ...
    cargarMedicamentos(farmaciaId) {
        var token = $('meta[name="csrf-token"]').attr('content');  hdnIdAtencion
        ...
        if (farmaciaId) {
            $.ajax({
                ...
                success: function (response) {
                    console.log('ttt', response.medicamentos);
                    ...
                    if (response && response.medicamentos && response.medicamentos.length > 0) {
                        ...
                        // Actualizar Chosen (si es necesario)
                        $('#cboMedicamento').trigger('chosen:updated');
                    } else {
                        console.error('No hay medicamentos disponibles o la estructura es incorrecta:', response);
    ...
    ajustarAltura() {
        ...
        $(".auto-expand").each(function () {
            this.style.height = Math.max(alturaMax, 60) + "px";
        });
    },
    ...
    Eventos() {
        ...
        $('#tblDiagnosticosRecetaMedica tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                $(this).removeClass('selected');
            }
            else {
                DTDiagnosticosRecetaMedica.$('tr.selected').removeClass('selected');
                $(this).addClass('selected');
            }       
        });

        $("#b_descripciondiagnostico").autocomplete({
            source: "../ws/proa/buscar_cie10",
            minLength: 3,
            select: function(event, ui) {
                if (ui.item) {
                    console.log('encontradno diagnsoticos',ui)
        ...
        $("#b_descripciondiagnosticoInter").autocomplete({
            minLength: 3,
            source: function (request, response) {
                var token = $('meta[name="csrf-token"]').attr('content');
                var datosTabla = DTDiagnosticosRecetaMedica.rows().data().toArray();
        
                $.ajax({
                    url: "../ws/ce/buscar_cie10/evaluacion",
                    type: "POST", 
                    dataType: "json",
                    data: {
                        term: request.term, 
                        diagnosticosActuales: JSON.stringify(datosTabla),
                        _token: token
                    },
        ...
        $(".auto-expand").on("input", OrdenMedica.ajustarAltura);
        ...
        $('#btnConsultarOrdenMedica').on('click', async function () {
            var objrowTb = DTORdenesMedicas.row('.selected').data();
            console.log('datos Ordenesvvvv',objrowTb)
            if (!isEmpty(objrowTb)) {
...
<<<<<<< HEAD
                            await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, objrow.IdNumero);
=======
                            await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, Variables.NroEvaluacion);
...
>>>>>>> c43206bf175774a3bfc1e1fc1e2cd58a60d1e406
        ...
        $('#btnGuardarRecetas').on('click', async function () {
            ...
            if (datarec.length > 0 || datarec == true) {      
                ...
                console.log('datos interconsultas',resp1);
                resp = Array.isArray(resp) ? resp : [];
                resp1 = Array.isArray(resp1) ? resp1 : [];
        ...
        $("#btnGuardarEva").on('click', async function () {
            console.log('entro a ordende hosp');
            ...
            Cargando(1);
              Triaje.GuardarTriajeHospitalizacion(Variables.IdAtencion, Variables.IdServicio, EvaluacionHospitalizacion.IdNumero); //hospitalizacion
            let respOrdenes = await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, EvaluacionHospitalizacion.IdNumero)
            let respInterconsulta= await OrdenMedica.CargarOrdenesInterconsultaPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, EvaluacionHospitalizacion.IdNumero,Variables.IdAtencion)
           
           
            let respConsumoServ =  await ConsumoServicio.CargarOrdenesCPT(Variables.IdCuentaAtencion,EvaluacionHospitalizacion.IdNumero);

            console.log('respConsumoServ',respConsumoServ)

            //Triaje.GuardarTriajeHospitalizacionPorEvaluacion(Variables.IdAtencion, Variables.IdServicio, EvaluacionHospitalizacion.IdNumero);
            // console.log(resp)
            ...
            Cargando(0);
                ...
                console.log('datos  de  ordenes',respOrdenes);
                console.log('datos interconsultas',respInterconsulta);
                resp = Array.isArray(respOrdenes) ? respOrdenes : [];
                resp1 = Array.isArray(respInterconsulta) ? respInterconsulta : [];
            ...
            let data2 = respConsumoServ.concat(respInterconsulta)

            let dataAll =data2.concat(respOrdenes);
...
>>>>>>> c43206bf175774a3bfc1e1fc1e2cd58a60d1e406
        ...
        $('#tblOrdenesMedicas tbody').on('click', '.ImprimirRecetaSF', async function () {
            ...
            console.log('datos impresion',row);
            ...
            }else if(row.IdPuntoCarga==12){
                Cargando(1);
                $.ajax({
                    url: '/hospitalizacion/SolicitudInterconsultaRecetas/' + row.IdReceta,
                    method: 'GET',
                    success: function (response) {
        ...
        $('#btnGuardarPaquetes').on('click', function () {
            ...
            console.log('puntocargaaerr',puntocarga);
    ...
    */
    ...
    async agregarPaqueteDetalle(idFactPaquete) {
        console.log('agregarPaqueteDetalle');
            ...
            var objselec = $("#cboMedicamento").val();
                ...
                if ($('#txtCAntidadFarmacia').val() > 0 || $('#b_Cantidad').val() > 0) {
                    ...
                    if (productosNoAgregados.length > 0) {
                        ...
                        var listaHTML = '<h2 style="font-family: \'Arial\', sans-serif; font-size: 24px; color: #00BFFF; text-align: center;">Productos no añadidos<br>Stock 0</h2>';
                    
                   
                        productosNoAgregados.forEach(function (producto) {
                            listaHTML += '<li style="margin-bottom: 10px;">*' + producto + '</li>';
        ...
        if (idCatalogo == 12) {
                ...
                if (!isEmpty(objselec)) {
                    if (OrdenMedica.existeProd(objselec, idCatalogo)) {
                        ...
                        var resumenHistoriaClinica = $("#txtResumenHistoriaClinica").val().trim();
                        var motivoInterconsulta = $("#txtMotivoInterconsulta").val().trim();
                        var dxDescripcion = $("input[name='l_cie10[]']").map(function () {
                             return $(this).val().trim();
                            }).get().filter((value) => value !== "");
    
                        // Validación
                        if (resumenHistoriaClinica === "") {
                            OrdenMedica.alerta(2,"Debe ingresar el Resumen de Historia Clínica.");
                            return false;
                        }
                        if (motivoInterconsulta === "") {
                            OrdenMedica.alerta(2,"Debe ingresar el Motivo de Interconsulta.");
                            return false;
                        }
                        if (dxDescripcion.length === 0) {
                            OrdenMedica.alerta(2, "Debe ingresar al menos un Diagnóstico.");
                            return false;
                        }
                        ...
                        $("#txtResumenHistoriaClinica, #txtMotivoInterconsulta").val("");
                        OrdenMedica.ajustarAltura();
                        $("#div_dx").empty();
    ...
    escapeHTML(str) { // Para recuperar los saltos de línea
        return String(str)
            .replace(/&/g, "&amp;")
            .replace(/"/g, "&quot;")
            .replace(/'/g, "&#39;")
        ...
        switch (idCatalogo) {
            ...
            case 12: 
                ...
                datainterconsulta.each(function (value, index) {
                    ...
                    var txtCant = "#txtCant_" + datainterconsulta[index]["idItem"];
                    var cboDx = "#cboDx_" + datainterconsulta[index]["idItem"];
                    html += '<RecetaDetalle idItem="' + datainterconsulta[index]["idItem"] + 
                    '" cantidadPedida="' + 1 + 
                    '" precio="' + datainterconsulta[index]["precio"] + 
                    '" total="' + 1*datainterconsulta[index]["precio"] + 
                    '" idDosisRecetada="" idViaAdministracion="" observaciones="' + "" + 
                    '" dx="' + datainterconsulta[index]["dx"] + 


                    '" resumenHistoriaClinica="' + OrdenMedica.escapeHTML(datainterconsulta[index]["resumenHistoriaClinica"]) +
                    '" motivoInterconsulta="' + OrdenMedica.escapeHTML(datainterconsulta[index]["motivoInterconsulta"]) +


                    
                    '" tiposDx="' + datainterconsulta[index]["tiposDx"] + '"></RecetaDetalle>';
                    /*html += '{"idItem":"' + datainterconsulta[index]["idItem"] + '","cantidadPedida":"' + $(txtCant).val() +
                        '","precio":"' + datainterconsulta[index]["precio"] + '","total":"' + datainterconsulta[index]["total"] +
                        '","idEspecialidad":"' + datainterconsulta[index]["idEspecialidad"] + '", "otraEspecialidad":"' + datainterconsulta[index]["otraEspecialidad"] + '"},';     */  //KHOYOSI                    
                });
...
>>>>>>> c43206bf175774a3bfc1e1fc1e2cd58a60d1e406
        ...
        console.log('ListaRecetaDetalleInterconsulta',ListaRecetaDetalleInterconsulta)
        ...
        formData.append("IdCama", aten.CodigoCama);
        ...
        formData.append('NroEvaluacion', EvaluacionHospitalizacion.IdNumero);
        console.log('OrdenMedica.js - GuardarOrdenesMedicas - NroEvaluacion', EvaluacionHospitalizacion.IdNumero);
...
>>>>>>> c43206bf175774a3bfc1e1fc1e2cd58a60d1e406
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/Recetas/GuardarRecetaHospi",
                    data: formData,
                    dataType: "json",
                    cache: false,
                    processData: false,
            ...
            columns: [
                ...
                },
                {
                    ...
                    createdCell: function (td, cellData, rowData, row, col) {
                        ...
                        if(rowData.IdPuntoCarga == 12)
                            {
                                $(td).html("INTERCONSULTA");
                            }
...
initDatablesTomografia() {
    ...
    async CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(idCuenta, nroEvaluacion) { // KHOYOSI
        ...
        try {
            ...
            datos = await
                ...
                resp=datos.resultado;
                console.log('Recetas/ListarOrdenesMedicasPorEvaluacion', resp);
    ...
    async SeleccionarRecetaDetalleHosp(idReceta, idPuntoCarga) {
        var resp = false;
        let datos
        var data = new FormData();
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/Recetas/ListaRecetaDetalleHosp",
            ...
            switch (idPuntoCarga) {
                ...
                case 22:
                    ...
                    break;
                case 12:
                    console.log('datos.resultado',datos.resultado)
                    if (!isEmpty(datos.resultado)) {
                        if (datos.resultado.length !== 0)
     ...
     async SeleccionarRecetaDetalle(idReceta, idPuntoCarga) {
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/Recetas/ListaRecetaDetalleInterconsulta",
                    //contentType: "application/json; charset=utf-8",
                    data: data,
                    dataType: "json",
                    cache: false,
    ...
    CargarCabeceraPorPuntoCarga(datos) {
        ...
        switch (parseInt(datos.IdPuntoCarga)) {
            ...
            case 12:
                ...
                $('#hdIdRecetaInterconsulta1').val(datos.idReceta);
                $('#txtResumenHistoriaClinica1').val(datos.ResumenHistoriaClinica);
                $('#txtMotivoInterconsulta1').val(datos.MotivoInterconsulta);
        ...
        if (datos.length > 0) {            
            $(datos).each(async function (i, obj) {
                ...
                if (obj.IdPuntoCarga == 21) //rx
                {
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 21)
                ...
                if (obj.IdPuntoCarga == 23) {//eco obs
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 23)
                    if (parseInt(obj.idEstado) === 1) {
                ...
                if (obj.IdPuntoCarga == 24) {//eco obs proc
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 24)
                    if (parseInt(obj.idEstado) === 1) {
                ...
                if (obj.IdPuntoCarga == 20) {//eco gene
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 20)
                    if (parseInt(obj.idEstado) === 1) {
                ...
                if (obj.IdPuntoCarga == 2) {//pt clinica
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 2)
                ...
                if (obj.IdPuntoCarga == 3) {//anat patologica
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 3)
                    if (parseInt(obj.idEstado) === 1) {
                ...
                if (obj.IdPuntoCarga == 11) {//sangre
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 11)
                ...
                if (obj.IdPuntoCarga == 22) {//tomografia jdelgado
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 22)
                    if (parseInt(obj.idEstado) === 1) {
                ...
                if (obj.IdPuntoCarga == 12) {//interconsulta jdelgado
                  //  $('#lblInterconsulta').html(labelReceta);
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 12)
...
>>>>>>> c43206bf175774a3bfc1e1fc1e2cd58a60d1e406
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleHosp(obj.IdReceta, 5)
                    if (parseInt(obj.idEstado) === 1) {
...
>>>>>>> c43206bf175774a3bfc1e1fc1e2cd58a60d1e406
    ...
    LimpiarOrdenesMedicasCE() {
     ...
     ad() {
        console.log('b_IdDiagnostico',$("#b_IdDiagnostico").val())
        console.log('b_idtipodiagnostico',$("#b_idtipodiagnostico").val())
        if ($("#b_IdDiagnostico").val() === null || $("#b_IdDiagnostico").val().length == 0 || $("#b_idtipodiagnostico").val().length == 0)
            ...
            $("#div_dx").append('<div class="row mx-0 d-flex justify-content-center align-items-center"> \
                <div class="col-lg-1 pr-0"> \
                    <input type="hidden" name="l_IdDiagnostico[]" value="' + $("#b_IdDiagnostico").val() + '"/> \
                    <input type="text" name="l_cie10[]" class="form-control form-control-sm" value="' + $("#b_cie10").val().trim() + '" disabled /> \
                </div> \
                <div class="col-lg-7 px-0"> \
                    <input type="text" name="l_descripciondia[]" class="form-control form-control-sm" value="' + $("#b_descripciondiagnosticoInter").val().trim() + '" disabled /> \
                </div> \
                <div class="col-lg-2 px-0"> \
                    <input type="hidden" name="l_idtipodiagnostico[]" value="' + $("#b_idtipodiagnostico").val().trim() + '"/> \
                    <input type="text" name="l_tipodiagnostico[]" class="form-control form-control-sm" value="' + $("#b_idtipodiagnostico option:selected").html() + '" disabled /> \
                </div> \
                <div class="col-lg-1 pl-0"> \
                    <input type="button" value="Eliminar" class="elimina_d form-control btn btn-sm btn-danger"/> \
                </div> \
            </div>');
    
            $("#b_IdDiagnostico").val("");
            $("#b_cie10").val("");
            $("#b_descripciondiagnosticoInter").val("");
            $("#b_iddiagnostico").val("");
            $("#b_masdetdia").val("");
        }
    },
    ...
    ad() {
        ...
        var objselec = DTDiagnosticosRecetaMedica.row('.selected').data();
        console.log('DTDiagnosticosRecetaMedica',objselec)

        $("#b_IdDiagnostico").val(objselec.idDiagnostico);
        $("#b_cie10").val(objselec.codigo);
        $("#b_descripciondiagnostico").val(objselec.diagnostico); 
		...
		if ($("#b_IdDiagnostico").val() === null || $("#b_IdDiagnostico").val().length == 0 || $("#b_idtipodiagnostico").val().length == 0)
			...
			$("#div_dx").append('<div class="row mx-0 d-flex justify-content-center align-items-center"> \
								<div class="col-lg-1 pr-0"><input type="hidden" name="l_IdDiagnostico[]" value="' + $("#b_IdDiagnostico").val() + '"/><input type="text" name="l_cie10[]" class="form-control form-control form-control-sm" value="' + $("#b_cie10").val().trim() + '" disabled /></div> \
								<div class="col-lg-7 px-0"><input type="text" name="l_descripciondia[]" class="form-control form-control-sm" value="' + $("#b_descripciondiagnostico").val().trim() + '" disabled /></div> \
								<div class="col-lg-2 px-0"><input type="hidden" name="l_idtipodiagnostico[]" value="' + $("#b_idtipodiagnostico").val().trim() + '"/><input type="text" name="l_tipodiagnostico[]" class="form-control form-control form-control-sm" value="' + $("#b_idtipodiagnostico option:selected").html() + '" disabled /></div> \
								<div class="col-lg-1 pl-0"><input type="button" value="Eliminar" class="elimina_d form-control btn btn-sm btn-danger"/></div>\
							</div>');
			$("#b_IdDiagnostico").val("");
			$("#b_cie10").val("");
			$("#b_descripciondiagnostico").val("");
			$("#b_iddiagnostico").val("");
			$("#b_masdetdia").val("");
		}
	},
