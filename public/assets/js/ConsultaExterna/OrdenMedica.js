...
var OrdenMedica = {
            ...
            $.ajax({
                ...
                success: function(resp){
                    ...
                    if(parseInt(result) <= 0)
                        ...
                        s_stock.className = "";
                    s_stock.innerText = parseInt(result);
                }
            });
        } catch (error) {
            console.error(error)
    ...
    Eventos() {
        ...
        $('#tblCatalogointerconsulta tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                $(this).removeClass('selected');
            }
            else {
                ...
                if (objrow.idEstado == 1) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, EvaluacionEmergencia.NroEvaluacion);
                        }                  
                    });
                }
                else {
        ...
        //         //await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencion(Variables.IdCuentaAtencion);
        //         // await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, Variables.NroEvaluacion);
        //         //await OrdenMedica.CargarOrdenesMedicasPorIdCuentaAtencionPorEvaluacion(Variables.IdCuentaAtencion, EvaluacionEmergencia.IdNumero); //PRUEBA
        //         ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //     Cargando(0);
        //     let datos;
        //         idCuenta=Variables.IdCuentaAtencion;
        //         nroEvaluacion= EvaluacionEmergencia.IdNumero;
        ...
        //                 AbrirVisorRecetas(datos.resultado);
        ...
        $('#btnQuitarinterconsulta').on('click', function () {
            OrdenMedica.quitarProd(12);
        })
        $('#btnAgregainterconsulta').on('click', function () {
    ...
    async agregarProd(idCatalogo) {
        ...
        //Interconsulta
        if (idCatalogo == 12) {
            //var objselec = $("#cbointerconsultaCE option:selected").attr("idproducto")
            var objselec = $("#cbointerconsulta").val();

            if (!isEmpty(objselec)) {
                if (OrdenMedica.existeProd(objselec, idCatalogo)) {
                    alerta(2, $('#cbointerconsulta option:selected').text() + " ya fue agregado.");
                    return false;
                }
                else {
                    precioUnitario = $("#cbointerconsulta option:selected").data("precio"); 
                    var objRow = {
                        idItem: objselec,
                        producto: $('#cbointerconsulta option:selected').text(),
                        cantidadPedida: 1,
                        precio: precioUnitario,
        ...
        if (idCatalogo == 5) {
            ...
            if (OrdenMedica.existeProd(objselec, idCatalogo)) {
                ...
                if ($('#txtCAntidadFarmacia').val() > 0 || $('#b_Cantidad').val() > 0) {
                    ...
                    //DTRecetaFarmacia.fnAdjustColumnSizing();
                    ...
                    $('#cboVia').val(0);
                    /*
                    dataTableFarmacia = DTRecetaFarmacia.rows().data();
    ...
    async GuardarOrdenesMedicas() {
        ...
        && ListaRecetaDetalleEcoGeneral == "<ROOT></ROOT>" && ListaRecetaDetalleAnatoPatologica == "<ROOT></ROOT>" &&
            ListaRecetaDetallePatalogiaClinica == "<ROOT></ROOT>" && ListaRecetaDetalleBancoSangre == "<ROOT></ROOT>" && ListaRecetaDetalleFarmacia == "<ROOT></ROOT>" && ListaRecetaDetalleInterconsulta == "<ROOT></ROOT>" && ListaRecetaDetalleTomografia == "<ROOT></ROOT>") {
    ...
    IniciarDataTablesInterconsulta() {
        DTRecetaInterconsulta = $('#tblCatalogointerconsulta').DataTable({
            "scrollY": "145px",
            "scrollCollapse": true,
            "autoWidth": false,
            data: null,
...
initDatablesInterconsulta() {
    ...
    //tableWrapper.find('.dataTables_length select').select2(); // initialize select2 dropdown
    oTable_InterconsultaOrdRes = $("#tblCatalogoInterconsultaOrdRes").dataTable(parms);
...
initDatablesRayos() {
    ...
    async SeleccionarRecetaDetalle(idReceta, idPuntoCarga) {
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/Recetas/ListaRecetaDetalleCE",
            ...
            console.log('OrdenMedica.js - SeleccionarRecetaDetalle - Recetas/ListaRecetaDetalleCE DATOS:', datos);
            switch (idPuntoCarga) {
                case 21:
        ...
        try {
            ...
            switch (idPuntoCarga) {
                ...
                case 11:
                    ...
                    break;
                case 12:
                        oTableInterconsulta.fnClearTable()
                        oTableInterconsulta.fnDraw()
                        if (!isEmpty(datos.resultado)) {
                            if (datos.resultado.length !== 0)
                                oTableInterconsulta.fnAddData(datos.resultado)
                        }
                        break;                    
                case 22:
    ...
    async SeleccionarRecetaDetalleSeguimiento(idReceta, idPuntoCarga) {
        ...
        oTable_InterconsultaOrdRes.fnClearTable()
        ...
        try {
            ...
            switch (idPuntoCarga) {
                ...
                case 23:
                    if (!isEmpty(datos.resultado)) {
                        if (datos.resultado.length !== 0) {
                            oTable_ecoObsOrdRes.fnAddData(datos.resultado);
                        }
                    }
                    break;                
                case 20:
                ...
                case 12:
                    if (!isEmpty(datos.resultado)) {
                        if (datos.resultado.length !== 0)
                            oTable_InterconsultaOrdRes.fnAddData(datos.resultado);
                        }
                    break;
                // case 22:
                //     if (!isEmpty(datos.resultado)) {
                //         if (datos.resultado.length !== 0)
                //             otable.rows.add(datos.resultado).draw();
                //     }
                //     break;
                case 5:
                    if (!isEmpty(datos.resultado)) {
    ...
    async ListarCabeceraRecetaByIdCuentaIdServicioIdMedico(idCuenta, idServicio, idMedico) { // KHOYOSI
        ...
        try {
            ...
            datos = await
                $.ajax({
                    method: "POST",
                    url: "/Recetas/ListarCabeceraRecetaByIdCuentaIdServicioIdMedicoCE",                    
                    data: data,
                    dataType: "json",
                    cache: false,
                    processData: false,
    ...
    CargarCabeceraPorPuntoCarga(datos) {
        ...
        $('[href="#interconsulta"]').closest('li').hide();
        ...
        switch (parseInt(datos.IdPuntoCarga)) {
            ...
            case 12:
                ...
                $('.nav-tabs a[href="#interconsulta"]').tab('show');
                $('[href="#interconsulta"]').closest('li').show();
                break;
            case 5:
    ...
    CargarDatosRecetaCabecera(datos) {
        console.log('CargarDatosRecetaCabecera: Hola');
        var labelReceta = "";
        OrdenMedica.LimpiarOrdenesMedicas();        
        if (datos.length > 0) {            
            $(datos).each(async function (i, obj) {
                ...
                if (obj.IdPuntoCarga == 12) {//interconsulta jdelgado
                    ...
                    if (parseInt(obj.idEstado) === 1) {
                        $('#btnAgregainterconsulta').css("visibility", 'visible');
                        $('#btnQuitarinterconsulta').css("visibility", 'visible');
                    }
                    else {
                        $('#btnAgregainterconsulta').css("visibility", 'hidden')
                        $('#btnQuitarinterconsulta').css("visibility", 'hidden')
                        //$('#lbltomografia').html("Receta Nro.: " + obj.idReceta + ' - Estado: Despachada');
                    }
                }
        ...
        if (datos.length > 0) {            
            $(datos).each(async function (i, obj) {
                ...
                if (obj.IdPuntoCarga == 12) {//Interconsulta
                    ...
                    await OrdenMedica.SeleccionarRecetaDetalleCE(obj.IdReceta, 12)
                    ...
                    if (parseInt(obj.idEstado) === 1) {
                        ...
                        $('#btnAgregainterconsulta').css("visibility", 'hidden')
                        $('#btnQuitarinterconsulta').css("visibility", 'hidden')
                        //$('#lblbancoInterconsulta').html("Receta Nro.: " + obj.idReceta + ' - Estado: Despachada');
                    }
                }                
                if (obj.IdPuntoCarga == 22) {//tomografia jdelgado
                    $('#lbltomografia').html(labelReceta);
    ...
    LimpiarOrdenesMedicas() {
        ...
        $('#txtCAntidadPatoClinica').val(1);
        $('#txtCAntidadbancoSangre').val(1);
        $('#txtCAntidadinterconsulta').val(1);
        $('#txtCAntidadtomografia').val(1);
    ...
    CargarDatosRecetaDetalle(datos) {
        ...
        $('#hdIdRecetaInterconsultaOrdRed').val(0)
        $('#lblinterconsulta').html(0)
            ...
            $(datos).each(function (i, obj) {
                ...
                if (obj.IdPuntoCarga == 12) {//interconsulta
                    ...
                    if (obj.idEstado === 1) {
                        ...
                        $('#lblinterconsultaOrdRes').html("Receta Nro.: " + obj.IdReceta + ' - Estado: Despachada');
                    }
                }                
                if (obj.IdPuntoCarga == 5) {//farmacia
