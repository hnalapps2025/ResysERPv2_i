...
var Emergencia = {
    ...
    async CargaInicial() {
        ...
        var fechaP = `${diaHoy}/${mesHoy}/${anioHoy}`;
        ...
        var mesPasado = fechaPasada.getMonth() + 1;
        var anioPasado = fechaPasada.getFullYear();
    ...
    Eventos() {
        ...
        $('#btnTodos').click(function () {
            ...
            $('#tblAtencionEmer').DataTable().column(7).search('').draw();
            // $("#btnBuscarAtencionesEmergencia").click();
        });

        // Botón "Atendido"
        $('#btnConAlta').click(function () {
            //oTable_InterconsultasHospitalizacion.column('name:idEstado').search('2').draw();  // Filtra por estado 2 (Atendido)
            $('#tblAtencionEmer').DataTable().column(7).search('1').draw();
        });

        // Botón "Todos"
        $('#btnSinAlta').click(function () {
            //oTable_InterconsultasHospitalizacion.column('name:idEstado').search('').draw();  // Muestra todos los datos
            $('#tblAtencionEmer').DataTable().column(7).search('0').draw();
        });
        ...
        $("#btnModificarAtenciones").on('click', async function () {
            ...
            var objrowTb = DTEmergencia.row('.selected').data();
            console.log('datos as', objrowTb);
            let fechaActual = new Date();
            let primerDiaMes = new Date(fechaActual.getFullYear(), fechaActual.getMonth(), 1);

            let opciones = { day: '2-digit', month: 'long' };
            let primerDiaFormato = primerDiaMes.toLocaleDateString('es-ES', opciones);
            let diaActualFormato = fechaActual.toLocaleDateString('es-ES', opciones);

            if (objrowTb) {
                let Frecuencia = await Emergencia.PacienteFrecuente(objrowTb.IdPacienteTriaje);
                if (Frecuencia.totalAtenciones >= 4) {
                    //alerta(2, `El Paciente tiene <br>${Frecuencia.totalAtenciones}</br> atenciones en los ultimos 7 dias.`);
                    Swal.fire({
                        title: 'Alert',
                        html: 'El Paciente tiene <b>' + Frecuencia.totalAtenciones + ' atenciones </b> en los ultimos <b> 7  dias. </b>',
                        icon: 'warning',
                        confirmButtonText: 'Aceptar'
                    });
                    setTimeout(() => {
                        window.location = "../../EvaluacionEmergencia/" + objrowTb.IdAtencion;
                    }, 2500);
    ...
    async PacienteFrecuente(IdPacienteTriaje) {
        try {
            Cargando(1);
            let token = $('meta[name="csrf-token"]').attr("content");
            let response = await $.ajax({
                method: "POST",
                url: "/EvaluacionEmergencia/PacienteFrecuente",
                data: {
                    IdPacienteTriaje: IdPacienteTriaje,
                    _token: token
                },
                dataType: "json"
            });
            ...
            Cargando(0);
            alerta('error', 'Error en la consulta', error);
            return [];
        }
    }
